...
const handleSubmit = async (e) => {
  ...
  'X-CSRF-TOKEN': csrfToken
    ...
    if (response.ok) {
      const data = await response.json();
      console.log(data.message); // Tampilkan pesan sukses dari server
      // Lakukan tindakan lain jika diperlukan, seperti menutup modal atau mengosongkan form
...
const handleUpdate = (row) => {
  setSelectedRow(row); // Simpan data karyawan yang akan diedit ke state `selectedRow`
  setEditOpen(true); // Buka modal edit
  ...
  if (karyawanToEdit) {
    setFormData({
      ...
      name: karyawanToEdit.nama,
      nik: karyawanToEdit.nik,
      no_tlp: karyawanToEdit.notelpon,
      jabatan: karyawanToEdit.jabatan,
      bidang: karyawanToEdit.bidang,
      password: '', // Kosongkan password jika tidak ingin mengubahnya
    });
  }
};
const handleSubmitUpdate = async (e) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      setEditOpen(false);
      fetchKaryawan(); // Refresh data karyawan setelah berhasil diupdate
    } else {
              ...
              <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center">
                < animated.div style={closingEditPopup ? closingEditPopupAnimation : popupEditAnimation } className="bg-white shadow-md rounded-lg px-8 py-6 max-w-xl md:w-[800px] w-full relative"> {/* Atur nilai max-w-xl atau max-w-3xl */}
                  ...
                  <form onSubmit={handleSubmitUpdate} className="grid grid-cols-2 gap-4">
                     <div className="mb-2">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Username</label>
                      <input type="text" id="username" name='username' value={formData.username} onChange={handleUpdate} className="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500" placeholder="username" required/>
                    </div>
                    <div className="mb-2">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nama</label>
                      <input type="text" id="nama" name='name' value={formData.name} onChange={handleUpdate} className="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500" placeholder="nama" required/>
                    </div>
                    <div className="mb-1">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">NIK</label>
                      <input name='nik' value={formData.nik} onChange={handleUpdate} type="text" id="NIK" className="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500" placeholder="NIK" required/>
                    </div>
                    <div className="mb-2">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">No. Telpon</label>
                      <input name='no_tlp' value={formData.no_tlp} onChange={handleUpdate} type="text" id="no.telpon" className="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500" placeholder="No. Telpon " required/>
                    </div>
                    <div className="mb-2">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Jabatan</label>
                      <input type="text" name='jabatan' value={formData.jabatan} onChange={handleUpdate} id="jabatan" className="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500" placeholder="Jabatan" required/>
                    </div>
                    <div className="mb-2">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Bidang</label>
                      <input name='bidang' value={formData.bidang} onChange={handleUpdate} type="text" id="bidang" className="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500" placeholder="Bidang" required/>
                    </div>
                    <div className="mb-2">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Password</label>
                      <input name='password' value={formData.password} onChange={handleUpdate} type="password" id="password" className="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500" placeholder="password" required/>
                    </div>
                    <div></div>
                    <button type="reset" className="button-masuk col-span-2 md:col-span-1 w-full md:w-auto flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">Reset</button>
                    <button type="submit" className="button-masuk col-span-2 md:col-span-1 w-full md:w-auto flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">Simpan</button>
